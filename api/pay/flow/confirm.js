...
async function tryActivateMembership({ email, planId, commerceOrder }) {
  try {
    // Activate membership in our DB (memberships table)
    ...
    console.log("[FLOW_CONFIRM] membership upserted", { email, planId, commerceOrder });
  } catch (e) {
    console.warn("[FLOW_CONFIRM] membership upsert failed", e && (e.message || e));
  }
}
